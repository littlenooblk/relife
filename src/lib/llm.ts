...
export async function generateOpeningLife(): Promise<StoryState> {
  ...
  const payload = await callOpenAiCompatibleApi([
    ...
    {
      role: "user",
      content: `请为一局新游戏生成一个高随机性的三国前中期开局，包括出生信息、家庭背景、初始关系、性格、资源、角色六维和第一组重大人生选择。本次随机种子：${Date.now()}-${Math.random().toString(36).slice(2)}。`,
    },
  ]);
...
7. 玩家本次行动是最高优先级。下一段剧情必须从这个选择的直接后果展开，不能忽略、反转或沿着你预设的其他路线推进；如果这个选择代价很高，可以写失败、误判或反噬，但必须让玩家清楚看到“这是因为我做了这个选择”。
8. 如果玩家角色死亡或人生已经结束，必须直接、克制地收束，不要拖沓，不要再安排新的冒险。
9. 一局游戏应在 16 到 32 个重大转折点内完成完整人生。第 16 个节点之后，可以根据剧情自然收束；第 32 个节点前后必须结束，避免无限铺陈。
10. 不要只写官场风险。人生节点必须轮换覆盖亲情、婚姻、爱情、伦理困境、宗族责任、养育、疾病、饥荒、迁徙、财产、师友、仇怨、信义与生死。
11. 角色形象六维会影响剧情发展：仁德高者更容易得到托付和民心，也更常遇到牺牲困境；谋略高者更容易看到暗线和布局；勇武高者会得到战事机会也更易受伤；魅力高者更容易牵动爱情、盟友与人心；名望高者会被举荐、嫉恨或政治利用；资财高者能经营家业也会引来索取和掠夺。
12. 每次必须根据玩家本次选择和剧情结果判断角色形象六维变化，返回 personaDeltas。变化量可以为负数，必须体现取舍，不要所有维度都上涨。
13. 禁止上帝视角：不要提前剧透未来历史结局，不要直接揭示其他人物未表露的隐秘动机，不要写“你不知道的是”“多年后史书记载”“此举将注定”等跳出玩家体验的句子。历史背景只能作为玩家可感知的时局、传闻、官府告示、亲友讲述或事后回望。
14. narrative 中不要写精确年龄、精确日期或精确经过几年几个月；精确年龄和日期由系统 UI 根据 timeDeltaDays 计算显示。正文只使用“少年时”“而立前后”“这些年”“数年间”“暮年”等模糊时间感。
15. 语言要有沉浸感和身体感：可以写饥饿、寒暑、道路、气味、家人的神情、城邑声音、选择前的犹豫，但不要堆砌宏大总结。
16. 不玄幻，不出现现代物品。只返回 JSON，不要 Markdown，不要额外解释。

JSON 格式：
{
  "title": "不超过12字的节点标题",
  "narrative": "220到420字第二人称沉浸剧情。必须先回应玩家本次选择造成的直接后果，再概括亲身经历的一段人生，并把玩家带到当前重大转折点。不要上帝视角，不要写精确年龄或精确日期",
...
- narrative 不要写“你二十岁/某年某月/三年后”这类精确时间；如果需要表达时间，只能使用模糊说法，避免和系统显示的年龄日期冲突。
...
注意：上面的精确年月日和年龄是系统记录，只用于你理解时间线，不要在 narrative 中复述。系统 UI 会单独显示本节点的精确年龄和日期。

玩家本次已经做出的选择（最高优先级，必须严格遵循）：
${action}

请生成下一段人生重大转折点。剧情必须从这个选择的直接后果展开；如果它和你原本准备的走向冲突，以玩家选择为准。先自动写完这次选择之后玩家亲身经历或合理得知的中间人生剧情和小变动，再把玩家带到下一个必须亲自决定的大关口。不要让玩家选择琐碎细节，不要使用上帝视角，不要在正文写精确年龄或精确日期。`;
}

function buildOpeningSystemPrompt() {
  return `你是三国人生游戏的开局生成器。

目标：生成一个极具随机性但历史地理合理的开局。不要使用固定模板，不要总是士族、农户、军户，也不要反复使用孤儿或遗孤这种高戏剧模板；但允许一定概率出生在世家、高门、地方大族或名士家族之中。

出身随机规则：
- 大多数开局应有明确在世亲属，例如父母之一、兄弟姊妹、配偶候选、族叔、师父、养父母或同乡庇护人。
- 孤儿、遗孤、被收养者只能作为少数稀有开局使用；除非本次随机种子明显适合，否则不要选择。
- 出生在世家/高门/地方大族应作为正常但非多数的可能性出现。若选择世家开局，要体现门第资源、藏书/师承/婚姻网络、族中长辈期待，也要有政治站队、家族名声、婚约或继承压力。
- 优先从多种家庭结构中随机：小吏之家、边郡军户、船户、盐户、医家、巫祝旁支、工匠、商旅、佃户、赘婿家庭、地方豪强庶支、郡望世家旁支、高门嫡支、名士家族、流寓士人、归附部族、坞堡佃客、驿传人家、织户、猎户等。
- familyBackground 必须写出家庭成员、经济来源、地方处境、一个牵挂和一个隐患；不要只用“孤苦无依”概括。
...
5. narrative 不要写精确年龄或精确日期；精确年龄和日期由 UI 展示。可以用“少年时”“将近成丁”“刚能独当一面”等模糊表达。
6. 只返回 JSON。
